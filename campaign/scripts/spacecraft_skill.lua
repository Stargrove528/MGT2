...
function parseComponents()
	...
	local sSkillLine = getValue();
	local nOffset = 0;

	if sSkillLine:match("^%s*%(") then
		local sPrefix;
		sPrefix,sSkillLine = sSkillLine:match("^(%s*%([^)]*%))(.*)$");
		if (sSkillLine or "") == "" then
			return;
		end
		nOffset = #sPrefix;
	end
	...
	local aClauses, aClauseStats = StringManager.split(sSkillLine, ",;\r", true);

	-- Check each comma-separated string for a potential skill roll or auto-complete opportunity
	for i = 1, #aClauses do
		local nStarts, nEnds, sLabel, sSign, sMod = string.find(aClauses[i],
			"([%w%s%(%)]*[%w%(%)]+)%s*([%+%-�]?)(%d*)");
		if nStarts then
			...
			table.insert(aComponents, {nStart = aClauseStats[i].startpos + nOffset,
				nLabelEnd = aClauseStats[i].startpos + nEnds, nEnd = aClauseStats[i].endpos + nOffset,
				sLabel = StringManager.capitalize(sLabel), nMod = nMod, nAllowRoll = nAllowRoll });
		end
	end
...
function action(draginfo)
    if sDragLabel then
        ...
        rAction.sActionName = sActionName;
        rAction.sSkillName = sDragLabel;
        rAction.nSkillMod = nDragMod;

		ActionShipSkill.performRoll(draginfo, rActor, rAction);
	end
end
